...
ENABLE_SCHEMA = cv.make_entity_service_schema({})
DISABLE_SCHEMA = cv.make_entity_service_schema({})
...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    async def async_enable_quiet_mode(call: ServiceCall) -> None:
        ...
        for entity_id in entity_ids:
            ...
            if state is None:
                # Not logging warning for Spotify/others if they are just unavailable/missing
                # to keep logs clean, but debug is useful.
            ...
            # Set the new volume
            # Catch errors (e.g. if device goes offline mid-call)
            try:
    ...
    async def async_disable_quiet_mode(call: ServiceCall) -> None:
        ...
        # We restore whatever we have stored, regardless of current selection
        # This handles the case where user unchecks a box WHILE quiet mode is on.
        # We still want to restore that entity.
